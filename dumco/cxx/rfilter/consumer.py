...
class ConsumerCoder(object):
    ...
    def _generate_class_decl(self, sf):
        ...
        if self.ct.mixed:
            if not self.gd.om.is_opaque_ct_member(self.ct, self.ct.text):
                vsf << valid << nl
        ...
        for (_, a) in dumco.schema.enums.enum_attribute_uses(self.ct):
            if not self.gd.om.is_opaque_ct_member(self.ct, a.attribute):
                vsf << valid << nl
        ...
        for (_, p) in dumco.schema.enums.enum_ct_particles(self.ct):
            if not self.gd.om.is_opaque_ct_member(self.ct, p.term):
                vsf << valid << nl
    ...
    def _generate_element_handler(self, sf, part):
        ...
        elif dumco.schema.checks.is_single_attribute_type(part.term.type):
            sf << 'void ' << self.gd.nm.set_leaf(self.ct, part.term) << '('
