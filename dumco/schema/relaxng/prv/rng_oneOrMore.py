...
def rng_oneOrMore(attrs, parent_element, builder, grammar_path, all_grammars):
...
class RngOneOrMore(base.RngBase):
    ...
    def finalize(self, grammar, builder):
        ...
        if len(self.patterns) > 1:
            ...
            group.finalize(grammar, builder)
            group.patterns = self.patterns
            self.patterns = [group]

        return super(RngOneOrMore, self).finalize(grammar, builder)
